...
from App.motor.Defaults import DEFAULT_PREFERENCES, DEFAULT_PROPELLANTS, KNSU_PROPS
...
def loadFile(path, dataType):
    with open(path, 'r') as readLocation:
        rawData = readLocation.read()

        # Conversions fro OpenMotor classes
        rawData = rawData.replace('uilib.fileIO.fileTypes', 'App.motor.FileIO.fileTypes')

        fileData = yaml.load(rawData, Loader=yaml.Loader)
