...
class CFDReport:
    ...
    def __init__(self, analysis):
        ...
        self._path = os.path.join(CfdTools.getOutputPath(self._analysis), translate('Rocket','CFD Report.docx'))
    ...
    def generateIntro(self):
        ...
        p = self._document.add_paragraph(translate('Rocket',"This is an automated report generated by the Rocket Workbench. "\
                                         "It was created by running a CFD analysis on the document '{}'.")\
                                            .format(FreeCAD.ActiveDocument.Label))

        self._document.add_paragraph(translate('Rocket',"Details about the study, including the rocket"))

        table = self._document.add_table(rows=4, cols=2, style='Table Grid')
        hdr_cells = table.columns[0].cells
        hdr_cells[0].text = translate('Rocket','Length')
        hdr_cells[1].text = translate('Rocket','Diameter')
        hdr_cells[2].text = translate('Rocket','Minimum Fin Thickness')
        hdr_cells[3].text = translate('Rocket','Frontal Area')
        ...
        self._document.add_heading(translate('Rocket','Study Parameters'), level=1)

        self._document.add_paragraph(translate('Rocket',"This study varied the following parameters:"))
        table = self._document.add_table(rows=1, cols=1, style='Table Grid')
        table.autofit = True
        hdr_cells = table.rows[0].cells
        hdr_cells[0].text = translate('Rocket','Angle of Attack')
        for angle in self._analysis.AOAList:
        ...
        if non_zero < 2:
            ...
            p.add_run(translate('Rocket',"NOTE: It is not possible to calculate the center of pressure at an "\
                                        "angle of attack of 0. It must be calculated at multiple points close "\
                                        "to 0 and inferred using l'Hôpital's rule.")).bold = True
            p = self._document.add_paragraph()
            p.add_run(translate('Rocket','This study is unable to determine Center of Pressure at a 0 angle of attack.')).italic = True

        self._document.add_heading(translate('Rocket','Configuration'), level=1)
    ...
    def physicsModelConfig(self):
        self._document.add_heading(translate('Rocket','Physics Model'), level=2)
        ...
        if physicsModel.SRFModelEnabled and physicsModel.Phase != "FreeSurface":
            ...
            row_cells[0].text = translate('Rocket',"Moving Reference Frame (SRF)")

            row_cells = table.add_row().cells
            row_cells[0].text = translate('Rocket',"RPM")
            row_cells[1].text = physicsModel.SRFModelRPM.UserString

            row_cells = table.add_row().cells
            row_cells[0].text = translate('Rocket',"Center of Rotation")
            ...
            row_cells = table.add_row().cells
            row_cells[0].text = translate('Rocket',"Rotational Axis")
            row_cells[1].text = "({}, {}, {})".format(
    ...
    def fluidPropertiesConfig(self):
        self._document.add_heading(translate('Rocket','Fluid Properties'), level=2)

        fluidModel = CfdTools.getMaterials(self._analysis)
        if len(fluidModel) == 1:
            fluidModel = fluidModel[0]
        else:
            self._document.add_paragraph(translate('Rocket',"Not defined."))
            return
        ...
        row_cells = table.rows[0].cells
        row_cells[0].text = translate('Rocket',"Material Name")
        row_cells[1].text = fluidModel.Label

        row_cells = table.add_row().cells
        row_cells[0].text = translate('Rocket',"Type")
    ...
    def initializeFlowConfig(self):
        self._document.add_heading(translate('Rocket','Initialize Flow Field'), level=2)
        ...
        row_cells = table.rows[0].cells
        row_cells[0].text = translate('Rocket',"Velocity")
        if initModel.PotentialFlow:
            row_cells[1].text = translate('Rocket',"Potential flow")
        elif initModel.UseInletUValues:
            row_cells[1].text = translate('Rocket',"Use values from boundary '{}'").format(initModel.BoundaryU.Label)
        else:
            row_cells[1].text = "({}, {}, {})".format(
        ...
        row_cells = table.add_row().cells
        row_cells[0].text = translate('Rocket',"Pressure")
        if initModel.PotentialFlowP:
            row_cells[1].text = translate('Rocket',"Potential flow")
        elif initModel.UseOutletPValue:
            row_cells[1].text = translate('Rocket',"Use values from boundary '{}'").format(initModel.BoundaryP.Label)
        else:
            row_cells[1].text = initModel.Pressure.UserString

        if physicsModel.Phase != 'Single':
            row_cells = table.add_row().cells
            row_cells[0].text = translate('Rocket',"Volume Fractions")

            if len(initModel.VolumeFractions) == 0:
                _msg("No fractions to report")
                row_cells[1].text = translate('Rocket',"None")
            else:
                row_cells[0].merge(row_cells[1])
        ...
        if physicsModel.Flow != "Isothermal":
            ...
            if initModel.UseInletTemperatureValue:
                row_cells[0].text = translate('Rocket',"Thermal")
                row_cells[1].text = translate('Rocket',"Use values from boundary '{}'").format(initModel.BoundaryT.Label)
            else:
                row_cells[0].text = translate('Rocket',"Temperature")
                row_cells[1].text = initModel.Temperature.UserString

        row_cells = table.add_row().cells
        row_cells[0].text = translate('Rocket',"Turbulence")
        if initModel.UseInletTurbulenceValues:
            row_cells[1].text = translate('Rocket',"Use values from boundary '{}'").format(initModel.BoundaryTurb.Label)
        else:
            row_cells[0].merge(row_cells[1])

            row_cells = table.add_row().cells
            row_cells[0].text = translate('Rocket',"Model") # This is a duplicate of the Physics model value
            row_cells[1].text = physicsModel.TurbulenceModel

            # NOTE: These value names should not be translated
    ...
    def solverConfig(self):
        self._document.add_heading(translate('Rocket','Solver'), level=2)
        ...
        row_cells = table.rows[0].cells
        row_cells[0].text = translate('Rocket',"Convergence tolerance")
        row_cells[1].text = str(solver.ConvergenceTol)

        row_cells = table.add_row().cells
        row_cells[0].text = translate('Rocket',"Max iterations")
        row_cells[1].text = str(solver.MaxIterations)

        row_cells = table.add_row().cells
        row_cells[0].text = translate('Rocket',"Iteration write interval")
        row_cells[1].text = str(solver.SteadyWriteInterval)

        if solver.Parallel:
            row_cells = table.add_row().cells
            row_cells[0].text = translate('Rocket',"Parallel cores")
            row_cells[1].text = str(solver.ParallelCores)

        row_cells = table.add_row().cells
        row_cells[0].text = translate('Rocket',"Starting from")
        row_cells[1].text = solver.StartFrom

        if physicsModel.Time == 'Transient':
            row_cells = table.add_row().cells
            row_cells[0].text = translate('Rocket',"End time")
            row_cells[1].text = solver.EndTime.UserString

            row_cells = table.add_row().cells
            row_cells[0].text = translate('Rocket',"Time step")
            row_cells[1].text = solver.EndTime.UserString

            row_cells = table.add_row().cells
            row_cells[0].text = translate('Rocket',"Max CFL number")
            row_cells[1].text = float(solver.MaxCFLNumber)

            row_cells = table.add_row().cells
            row_cells[0].text = translate('Rocket',"Max free-surface CFL number")
            row_cells[1].text = float(solver.MaxInterfaceCFLNumber)

            row_cells = table.add_row().cells
            row_cells[0].text = translate('Rocket',"Transient write interval")
            row_cells[1].text = solver.TransientWriteInterval.UserString

    def mesherConfig(self):
        self._document.add_heading(translate('Rocket','Mesher'), level=2)
        ...
        row_cells = table.rows[0].cells
        row_cells[0].text = translate('Rocket',"Mesh utility")
        row_cells[1].text = MESHER_DESCRIPTIONS[MESHERS.index(mesher.MeshUtility)]

        row_cells = table.add_row().cells
        row_cells[0].text = translate('Rocket',"Base element size")
        row_cells[1].text = mesher.CharacteristicLengthMax.UserString

        if mesher.MeshUtility == 'snappyHexMesh':
            row_cells = table.add_row().cells
            row_cells[0].text = translate('Rocket',"Cells between levels")
            row_cells[1].text = str(mesher.CellsBetweenLevels)

            row_cells = table.add_row().cells
            row_cells[0].text = translate('Rocket',"Relative edge refinement")
            row_cells[1].text = str(mesher.EdgeRefinement)

            row_cells = table.add_row().cells
            row_cells[0].text = translate('Rocket',"Edge detection")
            if mesher.ImplicitEdgeDetection:
                row_cells[1].text = translate('Rocket',"Implicit")
            else:
                row_cells[1].text = translate('Rocket',"Explicit")

        row_cells = table.add_row().cells
        row_cells[0].text = translate('Rocket',"STL relative linear deflection")
        row_cells[1].text = str(mesher.STLRelativeLinearDeflection)

        row_cells = table.add_row().cells
        row_cells[0].text = translate('Rocket',"STL angulat mesh density")
        row_cells[1].text = str(mesher.STLAngularMeshDensity)

        row_cells = table.add_row().cells
        row_cells[0].text = translate('Rocket',"Convert to dual mesh")
        row_cells[1].text = self.configState(mesher.ConvertToDualMesh)

        self._document.add_heading(translate('Rocket','Refinements'), level=3)
    ...
    def refinementConfig(self, refinement):
        ...
        row_cells = table.rows[0].cells
        row_cells[0].text = translate('Rocket',"Name")
        row_cells[1].text = refinement.Label

        row_cells = table.add_row().cells
        row_cells[0].text = translate('Rocket',"Refinement type")
        if refinement.Internal:
            row_cells[1].text = translate('Rocket',"Volume")

            row_cells = table.add_row().cells
            row_cells[0].text = translate('Rocket',"Relative element size")
            row_cells[1].text = str(refinement.RelativeLength)
        elif refinement.Extrusion:
            row_cells[1].text = translate('Rocket',"Extrusion")

            row_cells = table.add_row().cells
            row_cells[0].text = translate('Rocket',"Extrusion type")
            row_cells[1].text = EXTRUSION_NAMES[EXTRUSION_TYPES.index(refinement.ExtrusionType)]

            if refinement.ExtrusionType == "2DPlanar":
                row_cells = table.add_row().cells
                row_cells[0].text = translate('Rocket',"Thickness")
                row_cells[1].text = refinement.ExtrusionThickness.UserString

            elif refinement.ExtrusionType == "2DWedge":
                row_cells = table.add_row().cells
                row_cells[0].text = translate('Rocket',"Axis point")
                row_cells[1].text = "({} mm, {} mm, {} mm)".format(
                ...
                row_cells = table.add_row().cells
                row_cells[0].text = translate('Rocket',"Axis direction")
                row_cells[1].text = "({}, {}, {})".format(
                ...
                row_cells = table.add_row().cells
                row_cells[0].text = translate('Rocket',"Keep existing mesh")
                ...
                row_cells = table.add_row().cells
                row_cells[0].text = translate('Rocket',"Number of layers")
                row_cells[1].text = str(refinement.ExtrusionLayers)

                row_cells = table.add_row().cells
                row_cells[0].text = translate('Rocket',"Expansion ratio")
                ...
                row_cells = table.add_row().cells
                row_cells[0].text = translate('Rocket',"Angle")
            ...
            row_cells[1].text = translate('Rocket',"Surface")
            ...
            row_cells = table.add_row().cells
            row_cells[0].text = translate('Rocket',"Refinement thickness")
            row_cells[1].text = refinement.RefinementThickness.UserString

            if refinement.NumberLayers > 0:
                row_cells = table.add_row().cells
                row_cells[0].text = translate('Rocket',"Boundary layers")
                ...
                row_cells = table.add_row().cells
                row_cells[0].text = translate('Rocket',"Max first cell height")
    ...
    def boundaryConditionsConfig(self):
        self._document.add_heading(translate('Rocket','Boundary Conditions'), level=2)
    ...
    def bcConfig(self, bc):
        ...
        row_cells = table.add_row().cells
        row_cells[0].text = translate('Rocket',"Sub type")
        row_cells[1].text = SUBNAMES[typeIndex][SUBTYPES[typeIndex].index(bc.BoundarySubType)]

        row_cells = table.add_row().cells
        row_cells[0].text = translate('Rocket',"Default boundary")
    ...
    def bcWallConfig(self, table, typeIndex, bc):
        if bc.BoundarySubType == "partialSlipWall":
            row_cells = table.add_row().cells
            row_cells[0].text = translate('Rocket',"Slip ratio")
        ...
        if bc.BoundarySubType == "roughWall":
            row_cells = table.add_row().cells
            row_cells[0].text = translate('Rocket',"Roughness height (Ks)")
            row_cells[1].text = bc.RoughnessHeight.UserString

            row_cells = table.add_row().cells
            row_cells[0].text = translate('Rocket',"Roughness constant (Cs)")
    ...
    def bcInletConfig(self, table, typeIndex, bc):
        if bc.BoundarySubType == "uniformVelocityInlet" or bc.BoundarySubType == "staticPressureInlet":
            ...
            row_cells = table.add_row().cells
            row_cells[0].text = translate('Rocket',"Volume flow rate")
            ...
            row_cells = table.add_row().cells
            row_cells[0].text = translate('Rocket',"Mass flow rate")
    ...
    def bcVelocityConfig(self, table, bc):
        if bc.VelocityIsCartesian:
            ...
            row_cells = table.add_row().cells
            row_cells[0].text = translate('Rocket',"Face")
            row_cells[1].text = bc.DirectionFace

            row_cells = table.add_row().cells
            row_cells[0].text = translate('Rocket',"Inward normal")
    ...
    def bcTurbulenceConfig(self, table, bc):
        ...
        if bc.TurbulenceInletSpecification in ['intensityAndLengthScale']:
            row_cells[1].text = translate('Rocket',"Intensity & Length scale")

            row_cells = table.add_row().cells
            row_cells[0].text = translate('Rocket',"Turbulence intensity (I)")
            row_cells[1].text = bc.TurbulenceIntensityPercentage.UserString

            row_cells = table.add_row().cells
            row_cells[0].text = translate('Rocket',"Length scale (l)")
            row_cells[1].text = bc.TurbulenceLengthScale.UserString

        if bc.TurbulenceInletSpecification in ['TKEAndSpecDissipationRate']:
            row_cells[1].text = translate('Rocket',"Kinetic energy & Specific dissipation rate")

            row_cells = table.add_row().cells
            row_cells[0].text = translate('Rocket',"Turbulence kinetic energy (k)")
            row_cells[1].text = bc.TurbulentKineticEnergy.UserString

            row_cells = table.add_row().cells
            row_cells[0].text = translate('Rocket',"Specific dissipation rate (ω)")
            row_cells[1].text = bc.SpecificDissipationRate.UserString

        if bc.TurbulenceInletSpecification in ['TurbulentViscosityAndK']:
            row_cells[1].text = translate('Rocket',"Kinetic Energy & Turbulent viscosity")
            ...
            row_cells = table.add_row().cells
            row_cells[0].text = translate('Rocket',"Turbulent viscosity (v)")
    ...
    def configState(self, condition):
        if condition:
            return translate('Rocket',"True")
        else:
            return translate('Rocket',"False")

    def generateRuntime(self):
        self._document.add_heading(translate('Rocket','Run status'), level=1)

        self._document.add_paragraph(translate('Rocket',"Total run time: {}").format(self._totalRunTime))

        table = self._document.add_table(rows=1, cols=4, style='Table Grid')
        table.autofit = True
        hdr_cells = table.rows[0].cells
        hdr_cells[0].text = translate('Rocket','Angle of Attack')
        hdr_cells[1].text = translate('Rocket','Run Time')
        hdr_cells[2].text = translate('Rocket','Status')
        hdr_cells[3].text = translate('Rocket','Description')
        for angle in self._analysis.AOAList:
            row_cells = table.add_row().cells
            try:
                ...
                row_cells[0].text = str(angle)
                row_cells[1].text = ""
                row_cells[2].text = translate('Rocket',"Unknown")
                row_cells[3].text = ""

        self._document.add_page_break()

    def generateCP(self):
        self._document.add_heading(translate('Rocket','Center of Pressure'), level=1)
        p = self._document.add_paragraph("")
        p.add_run(translate('Rocket',"NOTE: It is not possible to calculate the center of pressure at an "\
                                    "angle of attack of 0. It must be calculated at multiple points close "\
                                    "to 0 and inferred using l'Hôpital's rule.")).bold = True
    ...
    def cpGraph(self, x_values, y_values):
        ...
        sub.set_xlabel(translate('Rocket',"Angle of Attack (degrees)"))
        sub.set_ylabel(translate('Rocket',"Center of Pressure (mm)"))
    ...
    def generateCD(self):
        self._document.add_heading(translate('Rocket','Lift and Drag'), level=1)
        p = self._document.add_paragraph(translate('Rocket',"Lift and Drag coefficients at multiple angles of attack with the rocket rotated "\
                                     "around the center point."))
    ...
    def cdGraph(self, x_values, cd_values, cl_values):
        ...
        sub.set_xlabel(translate('Rocket',"Angle of Attack (degrees)"))
        sub.set_ylabel(translate('Rocket',"Coefficient"))
        sub.grid(visible=True)
        sub.legend()
    ...
    def generateRunData(self):
        self._document.add_heading(translate('Rocket','Run Data'), level=1)
        for angle in self._analysis.AOAList:
            if self._runStatus[str(angle)][2] == "Success" or self._runStatus[str(angle)][3] == "Solver error":
                self._document.add_heading(translate('Rocket','Angle of Attack={}').format(str(angle)), level=1)
    ...
    def generateResidualDataAt(self, angle):
        ...
        sub.set_xlabel(translate('Rocket',"Iteration"))
        sub.set_ylabel(translate('Rocket',"Residual"))
        sub.set_yscale('log')
        sub.grid(visible=True)
        sub.legend()
    ...
    def generateForceDataAt(self, angle):
        ...
        sub.set_xlabel(translate('Rocket',"Iteration"))
        sub.set_ylabel(translate('Rocket',"Force [N]"))
        sub.grid(visible=True)
        sub.legend()
    ...
    def generateMomentDataAt(self, angle):
        ...
        sub.set_xlabel(translate('Rocket',"Iteration"))
        sub.set_ylabel(translate('Rocket',"Moment"))
        sub.grid(visible=True)
        sub.legend()
